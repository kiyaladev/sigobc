<template>
  <q-page class="q-pa-md">
    <PageHeader
      title="Gestion des Mandats de Recette"
      subtitle="Mandats de recette par taxe et bordereau"
      icon="description"
    >
      <template #actions>
        <q-btn color="primary" icon="add" label="Nouveau Mandat de Recette" @click="openDialog()" />
      </template>
    </PageHeader>
    ...
    <q-dialog v-model="dialogVisible" persistent>
      <q-card style="min-width: 600px; max-width: 90vw">
        <q-card-section class="bg-primary text-white row items-center">
          <div class="text-h6">{{ isEditing ? 'Modifier' : 'Nouveau' }} Mandat de Recette</div>
          <q-space />
...
async function onSubmit() {
  ...
  try {
    ...
    if (isEditing.value && form.value.id) {
      ...
      $q.notify({ type: 'positive', message: 'Mandat de Recette modifié' });
    } else {
      data.createdAt = now;
      await db.declarations.add(data as Declaration);
      $q.notify({ type: 'positive', message: 'Mandat de Recette créé' });
    }
...
function confirmDelete(declaration: Declaration) {
  $q.dialog({
    title: 'Confirmation',
    message: `Supprimer le mandat "${declaration.numeroPiece}" ?`,
    cancel: true,
    persistent: true,
  }).onOk(() => {
    void (async () => {
...
function printMandat(declaration: Declaration) {
  // Ouvrir le template HTML créé : mandat_recette.html
  window.open('/mandat_recette.html?declarationId=' + declaration.id, '_blank');
}

function downloadMandatPDF(declaration: Declaration) {
  window.open('/mandat_recette.html?declarationId=' + declaration.id + '&print=true', '_blank');
}
