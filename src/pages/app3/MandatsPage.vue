...
const importPreviewData = ref<ImportMandatRow[]>([]);
const importErrors = ref<string[]>([]);

interface ImportMandatRow {
  _rowIndex: number;
  _hasError: boolean;
...
const importPreviewColumns = [
  ...
  { name: 'beneficiaire', label: 'Bénéficiaire', field: 'beneficiaire', align: 'left' as const },
...
function downloadTemplate() {
  window.open('/templates/mandat_depense_template.csv', '_blank');
}
...
async function parseCSV(csvText: string) {
  ...
  const currentYear = new Date().getFullYear();
  let nextNumero = (await db.mandats.where('exercice').equals(currentYear).count()) + 1;
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (!row.beneficiaire) {
      hasError = true;
      errorMessage = 'Bénéficiaire requis';
    }
    if (!row.objet) {
      hasError = true;
    ...
    if (!numeroMandat) {
      numeroMandat = String(nextNumero++);
      autoNumero = true;
    }
    ...
    rows.push({
      ...
      sousChapitreCode: row.sousChapitreCode || '',
...
async function executeImport() {
  ...
  try {
    ...
    const personnelId = 1;
    let successCount = 0;
    const bordereauIdsToUpdate = new Set<number>();
    ...
    for (const row of importPreviewData.value) {
      ...
      // Trouver le sousChapitreId par code
      let sousChapitreId: number | undefined;
      if (row.sousChapitreCode) {
        const sousChapitre = sousChapitres.value.find((s) => s.code === row.sousChapitreCode);
        if (sousChapitre) sousChapitreId = sousChapitre.id;
      }

      // Trouver le bordereauMandatId par numéro
      let bordereauMandatId: number | undefined;
      if (row.bordereauNumero) {
        ...
        if (bordereau) {
          ...
          bordereauIdsToUpdate.add(bordereau.id!);
        }
      }
    ...
    // Recalculer les stats des bordereaux impactés
    for (const bordereauId of bordereauIdsToUpdate) {
      await recalculateBordereauStats(bordereauId);
    }

    $q.notify({
      type: 'positive',
      message: `${successCount} mandat(s) importé(s) avec succès`,
    });
